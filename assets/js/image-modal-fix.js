(() => {
  ...
  function ensureModal() {
    ...
    } else if (closeBtn.parentElement !== modal) modal.appendChild(closeBtn);
    if (!img) {
      img = document.createElement("img");
      img.id = "imageModalImg";
      img.alt = "Full size image";
      modal.appendChild(img);
    } else if (img.parentElement !== modal) modal.appendChild(img);
  ...
  function bindImages() {
    ...
    if (closeBtn.dataset.bound !== "1") {
      ...
      closeBtn.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); closeImage(); });
      closeBtn.addEventListener("pointerup", (e) => { e.preventDefault(); e.stopPropagation(); closeImage(); });
    ...
    document.querySelectorAll(".post-image,.card-image,.featured-image,.calendar-event-image,.public-gallery-image,.post-full-image,.news-image,.event-image,.announcement-image,img[data-fullscreen='true']").forEach((img) => {
      ...
      img.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); openImage(img.currentSrc || img.src); });
    });
