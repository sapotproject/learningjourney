...
function closeImageModal() {
  ...
  // Safety cleanup for mobile browsers after tapping Close.
  setTimeout(() => {
    modal.classList.remove("show");
    modal.setAttribute("aria-hidden", "true");
    document.body.classList.remove("image-modal-open");
...
const imageCloseBtn = gById("imageCloseBtn");
if (imageCloseBtn) {
  imageCloseBtn.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    closeImageModal();
  });

  imageCloseBtn.addEventListener("touchend", (event) => {
    event.preventDefault();
    event.stopPropagation();
    closeImageModal();
  }, { passive: false });
...
const imageModal = gById("imageModal");
if (imageModal) {
...
function groupGallery(photos) {
  ...
  const cats = [
    ...
    ...Array.from(map.keys()).filter((cat) => !order.includes(cat)).sort()
  ];
  ...
  return cats.map((cat) => `
    <section class="public-gallery-category">
      ...
      <div class="public-gallery-grid">
        ${map.get(cat).map(galleryCard).join("")}
      </div>
    </section>
  `).join("");
}
...
async function loadGallery() {
  ...
  try {
    ...
    if (!photos.length) {
      ...
      galleryList.innerHTML = groupGallery(photos);
    }

    galleryStatus.textContent = galleryData.generated_at
      ? "Last updated: " + new Date(galleryData.generated_at).toLocaleString("en-PH")
      : "";
  } catch (error) {
    galleryList.innerHTML = `
      <div class="empty-state">
